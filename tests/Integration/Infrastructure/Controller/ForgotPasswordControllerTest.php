...
final class ForgotPasswordControllerTest extends AbstractWebTestCase
{
    public function testForgotPassword(): void
    {
        ...
        $this->assertMetaTitle('Mot de passe oublié ? - Jeunot', $crawler);

        $saveButton = $crawler->selectButton('Envoyer un lien de réinitialisation');
        $form = $saveButton->form();
        $form['forgot_password_form[email]'] = 'mathieu@fairness.coop';
        $client->submit($form);
        ...
        $this->assertResponseStatusCodeSame(200);
        $this->assertSame('Si cette adresse e-mail est enregistrée chez Jeunot, vous recevrez un lien pour réinitialiser votre mot de passe.', $crawler->filter('div.alert--success')->text());
